...
int main() {
    ...
    ifstream file("sudokupuzzles.txt"); // файл из которого читаем (дл¤ линукс путь будет выгл¤деть по другому)

    while (getline(file, s)) { // пока не достигнут конец файла класть очередную строку в переменную (s)
        cout << s << endl; // выводим на экран
        s += "+"; // что нибудь делаем со строкой например ¤ добавл¤ю плюсик в конце каждой строки
